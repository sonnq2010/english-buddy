...
export class SocketMessageService {
  ...
  async joinRoom(client: WebSocket, server: Server, roomIdJoined?: string) {
    this.logger.log(`joinRoom clientId ${client.id}, roomIdJoined ${roomIdJoined}`)
    const roomEmpty = await this.roomService.findRoomEmpty(client, roomIdJoined);
    //join room in redis
    ...
    client.send(JSON.stringify(socketMessageDTO))
    this.sendMessageForAnotherInRoom(server, client, roomEmpty, JSON.stringify({type: 'text', data: `user ${client.id} joined room`}));
  }
  ...
  async leaveRoom(client: WebSocket, server: Server) {
    ...
    this.sendMessageForAnotherInRoom(server, client, roomId, JSON.stringify({type: 'text', data: `user ${client.id} out room`}));
    return roomId;
  }
  ...
  async sendMessageForAnotherInRoom(server: Server, client: WebSocket, roomId: string, message: string) {
